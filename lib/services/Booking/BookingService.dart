...
import 'package:wedding_planning_application/repository/Booking/getbookingcouple.dart';
...
class BookinigService {
  ...
  final Getbookingrepocouple getcou= Getbookingrepocouple();
...
Future<Showbooking> getserviceitem(int serviceitemid) async {
  ...
  Future<List<GetBookings>> getbookingcouple(int id ) async {
    List<GetBookings> getbooking = [];
    try {
      getbooking = await getcou.getbookingrepocouple(id);
    } catch (e) {
      // log(e.toString());
    }
// log(getbooking.toString());
